...
export const getStops = async (includeInactive = false): Promise<Stop[]> => {
  ...
  if (error) { console.error('Error fetching stops:', error); return []; }
  return data || [];
};
...
export const createStop = async (stop: Omit<Stop, 'id' | 'created_at' | 'updated_at'>): Promise<Stop | null> => {
  ...
  if (error) { console.error('Error creating stop:', error); throw error; }
  return data;
};
...
export const updateStop = async (id: string, updates: Partial<Stop>): Promise<Stop | null> => {
  ...
  if (error) { console.error('Error updating stop:', error); throw error; }
  return data;
};

export const deleteStop = async (id: string): Promise<boolean> => {
  const { error } = await supabase.from('stops').delete().eq('id', id);
  if (error) { console.error('Error deleting stop:', error); return false; }
  return true;
};
...
export const getRouteTemplates = async (includeInactive = false): Promise<RouteTemplate[]> => {
  ...
  const { data, error } = await query;
  if (error) { console.error('Error fetching route templates:', error); return []; }
  return data || [];
};
...
export const createRouteTemplate = async (
  ...
  if (error) { console.error('Error creating route template:', error); throw error; }
  return data;
};
...
export const updateRouteTemplate = async (
  ...
  if (error) { console.error('Error updating route template:', error); throw error; }
  return data;
};
...
export const deleteRouteTemplate = async (id: string): Promise<boolean> => {
  ...
  if (error) { console.error('Error deleting route template:', error); return false; }
  return true;
};
...
export const getRouteTemplateStops = async (templateId: string): Promise<RouteTemplateStop[]> => {
  ...
  if (error) { console.error('Error fetching route template stops:', error); return []; }
...
export const setRouteTemplateStops = async (
  ...
  // Delete existing stops for this template
  const { error: deleteError } = await supabase
    .from('route_template_stops')
    .delete()
    .eq('route_template_id', templateId);
  if (deleteError) { console.error('Error clearing template stops:', deleteError); return false; }
  ...
  if (insertError) { console.error('Error setting template stops:', insertError); return false; }
  return true;
};
